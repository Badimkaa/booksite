...
**Если через файлы:**
11.  Загрузите новые файлы (кроме `data`!) через Файловый менеджер с заменой.
2.  На сервере выполните те же команды (начиная с `npm install`).

## Шаг 8: Безопасность (Важно)

Чтобы защитить ваши данные (особенно `.env`):

1.  **Права доступа к файлу:**
    Сделайте так, чтобы файл `.env` мог читать только владелец (вы). Выполните на сервере:
    ```bash
    chmod 600 /root/app/.env
    ```

2.  **Если злоумышленник получит доступ:**
    Честно говоря, если хакер получит `root` доступ или доступ к процессу Node.js, он сможет прочитать переменные. Поэтому главная защита — не пускать никого на сервер:
    *   Используйте только вход по SSH ключам (пароли можно отключить).
    *   Не ставьте подозрительные плагины/библиотеки.
    *   Регулярно обновляйте систему (`apt update && apt upgrade`).

3.  **Если утечка случилась:**
    Сразу же перевыпустите все ключи (Telegram Bot Token, Prodamus Secret Key и т.д.). Старые ключи перестанут работать, и хакеры ничего не смогут с ними сделать.
```
